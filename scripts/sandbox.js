...
async function main() {
    ...
    const token = await Token.buildToken('Please enter your account address')
    console.log(token)
    console.log(token.format(BigNumber.from('166527132537660'), true))
}

function emulateTrade() {
    const tokenInfoA = { "address": "Please enter your account address", "name": "USD Coin", "symbol": "USDC", "decimals": 6 }
    const tokenInfoB = { "address": "Please enter your account address", "name": "Wrapped Ether", "symbol": "WETH", "decimals": 18 }
